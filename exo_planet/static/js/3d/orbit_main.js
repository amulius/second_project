;function Asterank3D(opts) {
  ...
  opts.default_camera_position = opts.camera_position || [0, 155 * 5, 32 * 5];
...
//  })();
  ...
  var stats, scene, renderer, composer, gui, GUItext, f1, f2
    ...
    , exoPlanetSize = 0
    , exoPlanetSizeOptions = { Accurate: 0, Earth: 1, Jupiter: 2, '3xJupiter': 3, '10xJupiter': 4 }
    , jed = toJED(new Date())
            ...
            $.ajax({
                ...
                success: function (data) {
                    for (var i = 0; i < data.systems.length; i++) {
                        var exo_data = {
                            full_name: data.systems[i].pl_name,
                            host_name: data.systems[i].pl_hostname,
                            ma: 0,        // mean anomaly
                            epoch: data.systems[i].pl_orbtper || 0,       //                                                              might be able to not need
                            a: data.systems[i].pl_orbsmax || 1,                //Semimajor axis                                                exoplanet might have
                            e: data.systems[i].pl_orbeccen || 0,               //Eccentricity                                                  exoplanet might have
                            i: data.systems[i].pl_orbincl || 0,               //Inclination                                                   exoplanet might have
                            w_bar: data.systems[i].pl_orblper || 0,     // longitude of perihelion(w + omega) Longitude of the periapsis    exoplanet might have
                            ...
                            P: data.systems[i].pl_orbper || 365.256,          //Period                                                        exoplanet might have
                            R: data.systems[i].pl_radj * 11.209 || 1                    // radius                                                       nice to have
                        };
...
//      console.log('create particles');
    ...
    uniforms = {
      ...
      planet_texture:
        { type: 't', value: loadTexture('static/img/cloud4.png') },
      small_roid_texture:
        { type: 't', value: loadTexture(opts.object_texture_path) },
      small_roid_circled_texture:
        { type: 't', value: loadTexture('static/img/cloud4-circled.png') }
    };
